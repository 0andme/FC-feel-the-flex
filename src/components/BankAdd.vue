<template>
  ...
  >
    ...
    <template #headerTitle>
      <<<<<<< HEAD
      <div>logo</div>
      ======= >>>>>>> 47e40b864cdad04d50094927c24cb5c0fda644de
      <p>결제 계좌 등록</p>
    </template>
    <template #default>
      <!-- 은행 선택 부분 -->
      <template v-if="!isNextBtnClick">
        ...
        <div class="all_bank_list">
          <template v-for="bankItem in Banklist" :key="bankItem.id">
            <BankAddItem
        ...
        <div class="control_btn">
          <button v-if="clickCount < 2" @click="isNextBtnClick = true">
            다음
          </button>
          <button v-if="clickCount < 2" @click="resetValue">취소</button>
      ...
      <template v-else>
        <div class="userDataInput">
          ...
          <div class="item">
            <div class="title">
              <span>계좌 번호</span>
              <span v-show="!checkAccount()" class="title_guide err">
                계좌 번호 길이를 확인 하세요</span
              >
              <span v-show="checkAccount()" class="title_guide">OK</span>
            </div>
            <input
              v-model="userAccount"
          ...
          <div class="item">
            <div class="title">
              <span>전화 번호</span>
              <span v-show="!checkPhoneNum()" class="title_guide err"
                >-를 빼고 입력해주세요</span
              >
              <span v-show="checkPhoneNum()" class="title_guide">OK</span>
            </div>
            <input v-model="userPhoneNum" type="text" @blur="checkPhoneNum" />
          </div>
          <!-- 계좌 등록 -->
          <div class="item">
            <div class="title">
              ...
              />
              <span>계좌 등록 동의</span>
              <span v-show="!checkSignature()" class="title_guide err"
                >(필수)</span
              >
              <span v-show="checkSignature()" class="title_guide">OK</span>
            </div>
          </div>
        </div>
        <div class="control_btn">
          <button @click="addAccount">등록</button>
          <button @click="resetValue">취소</button>
        </div>
      </template>
    </template>
  </PopupModal>
...
export default {
  ...
  computed: {
    Banklist() {
      return this.$store.state.bank.allBankList?.filter(
        (bank) => bank.disabled === false
      );
    },
    bankAccountLength() {
  ...
  methods: {
    ...
    checkPhoneNum() {
      return (
        this.userPhoneNum?.length === 11 &&
        this.userPhoneNum?.indexOf("-") === -1
      );
    },
    checkSignature() {
