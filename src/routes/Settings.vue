  ...
  <div class="settings_page">
    ...
    <div class="container">
      ...
      <SettingsItem :edit-data-function="editUserImg">
        <template #item_name> 프로필 사진 </template>
        ...
        <template #default>
          <div class="img_upload_wrap">
            ...
            />
            <div v-else class="settingsAfter user_profile__img src">
              <img :src="profileImgSrc" />
            </div>
            <div class="img_upload_text">
              <label>
                <input id="file_upload_btn" type="file" @change="selectFile" />
                파일 업로드
              </label>
      ...
      <SettingsItem :edit-data-function="editUserName">
        <template #item_name> 이름 </template>
        <template #beforChange>
      ...
      <SettingsItem :edit-data-function="editUserPw">
        <template #item_name> 비밀번호 </template>
        <template #default>
          ...
          <div class="form_items_wrap">
            <span>변경할 비밀번호</span>
            <span :class="[pwSameCheck() ? 'isOk' : '', 'pwCheckMsg']">{{
              pwSameCheckMsg
            }}</span>
            <input
              v-model="newUserPw"
    ...
    editUserPw() {
      ...
      if (this.pwSameCheck()) {
        ...
        if (confirm("비밀번호 변경시 재로그인이 필요합니다")) {
          editInfo({
            oldPassword: this.oldUserPw,
            newPassword: this.newUserPw,
          }).then(() => {
            // 서버에 로그아웃 요청
            logOut().then(() => {
              // 엑세스 토큰 삭제
...
.devider {
  width: 100%;
  border-bottom: 1px solid rgb(230, 230, 230); // 수정 색상 태그
  margin: 10px 0;
}
.container {
  width: 70%;
